...
public class MainHard : MonoBehaviour {
    ...
    private MissileControlHard[] rocketsControl;
    public float speed;
    ...
    void Update() {
        if (startSimulationBool == true) {
            ...
            if (SetUpRockets) {
                ...
                rocketsControl = new MissileControlHard[numRockets];
    ...
    float percentFinished(MissileControlHard[] mce) {
    ...
    void destroyAndCreate(List<MissileControlHard> matingpool, MissileControlHard mostFit) {
        ...
        rocketsControl[1] = rockets[1].GetComponentInChildren<MissileControlHard>();
    ...
    float[] mutate(float[] gene) {
        if (UnityEngine.Random.Range(0, 20) < mutationRate && currentMilestoneLevel < 34) {
