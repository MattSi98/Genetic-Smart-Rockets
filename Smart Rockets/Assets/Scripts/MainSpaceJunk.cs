...
public class MainSpaceJunk : MonoBehaviour {
    ...
    private MissileControlRandomSpaceJunk[] rocketsControl;
    public float speed;
    ...
    private GenerationDisplayRandom generationDisplay;
        ...
        foreach (Transform child in transform) {
            if (child.tag == "Canvas")
            {
                foreach (Transform grandChild in child)
                {
                    ...
                    {
                        generationDisplay = grandChild.gameObject.GetComponent<GenerationDisplayRandom>();
                    }
                }
            }
    ...
    void Update() {
        if (startSimulationBool == true) {
            ...
            if (SetUpRockets) {
                ...
                rocketsControl = new MissileControlRandomSpaceJunk[numRockets];
    ...
    float percentFinished(MissileControlRandomSpaceJunk[] mce) {
    ...
    void destroyAndCreate(List<MissileControlRandomSpaceJunk> matingpool, MissileControlRandomSpaceJunk mostFit) {
        ...
        rocketsControl[1] = rockets[1].GetComponentInChildren<MissileControlRandomSpaceJunk>();
    ...
    float[] mutate(float[] gene) {
        if (UnityEngine.Random.Range(0, 20) < mutationRate && currentMilestoneLevel < 7) {
